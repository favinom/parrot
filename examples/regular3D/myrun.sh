...
#parameters conductive fractures
poro=0.9
perm=1e4
#parameters blocking fractures
#poro=0.01
#perm=1e-4
...
if [ $correction -eq 0 ]
then
	mpirun -n ${np} ../../parrot-opt -i 3advection.i resolution=${res} unifSteps=${us} adaptSteps=${as} pf=${poro} kf=${perm} UserObjects/active='soln'
fi
if [ $correction -eq 1 ]
then
