...
//    auto _M  = storeOperatorsUO.MassMatrix();
//    auto _L  = storeOperatorsUO.LumpMassMatrix();
//    auto _J_stab  = storeOperatorsUO.JacMatrix();
//    auto _PL = storeOperatorsUO.PoroLumpMassMatrix();
//    auto _PM = storeOperatorsUO.PoroMassMatrix();
//    auto _D  = storeOperatorsUO.StabMatrix();
//
//    if(_fe_problem.timeStep()==1)
//    {
//        _J_stab->attach_dof_map(dof_map);
//        _J_stab->init();
//
//        std::vector<Real> v1;
//        std::vector<int> c1;
...
//        int rb1=petsc_mat->row_start();
//
//        int rb2=_J_stab->row_start();
//
//        int re1=petsc_mat->row_stop();
//
//        int re2=_J_stab->row_stop();
//
//        if (rb1!=rb2)
//        {
//            std::cout<<"rb\n";
...
//
//
//    std::vector<double>_vec_localize; ghosted_solution.localize(_vec_localize);
//
//    _L->get_diagonal(_inv);
//    _inv.reciprocal();
//
...
//    _inv_p.reciprocal();
//
//    PetscVector<Number> _u_dot_kuz(_pp_comm, dof_map.n_dofs(), dof_map.n_local_dofs());
//    _u_dot_kuz.zero();
//
//    PetscVector<Number> _u_dot_moose(_pp_comm, dof_map.n_dofs(), dof_map.n_local_dofs());
//    _u_dot_moose.zero();
//    //std::unique_ptr<NumericVector<Number>> _u_dot_kuz = ghosted_solution.zero_clone();
//
//    //std::unique_ptr<NumericVector<Number>> _u_dot_moose = ghosted_solution.zero_clone();
//
//    _J_stab->vector_mult(_u_dot_kuz,ghosted_solution);
//
//    _u_dot_moose.pointwise_mult(_inv,_u_dot_kuz);
//
//    //NumericVector<Number> * _u_dot_moose = _nl.solutionUDot();
//
//    PetscVector<Number> &_u_dot = dynamic_cast<libMesh::PetscVector<libMesh::Number>& >(_u_dot_moose);
//
//    std::vector<double>_vec_localize_dot; _u_dot.localize(_vec_localize_dot);
//
//
//    //    NumericVector<Number> * local_vector;
//    //
...
//
//
//    PetscMatrix<Number> _J_stab_tr(_pp_comm);
//    _J_stab->get_transpose(_J_stab_tr);
//
//    Mat PM_petsc    = _PM->mat();
//    //Mat L_petsc     = _L->mat();
//    Mat D_petsc     = _D->mat();
