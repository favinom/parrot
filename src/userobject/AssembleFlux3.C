...
registerMooseObject("parrotApp", AssembleFlux3);


template <>
InputParameters
validParams<AssembleFlux3>()
{
...
AssembleFlux3::AssembleFlux3(const InputParameters & parameters) :
...
AssembleFlux3::ComputeFlux()
{
    ...
    for ( ; el != end_el; ++el)
    {
       ...
       stab.zero();
       for (int iii=0; iii<ke.m(); ++iii)
        for (int jjj=iii+1; jjj<ke.m(); ++jjj)
        {
          if (iii!=jjj)
          {
            Real aij=ke(iii,jjj);
            Real aji=ke(jjj,iii);
            Real amax=std::max(aij,aji);
            if (amax>0.0)
            {
              stab(iii,jjj)=-amax;
              stab(jjj,iii)=-amax;
    ...
    AssembleFlux3::determine_dc_bnd_var_id(AssembleFlux3::split_string(_dc_var, ' '));
...
Real
AssembleFlux3::ComputeMaterialProprties(const Elem *elem)
{
...
void
AssembleFlux3::determine_dc_bnd_var_id(const std::vector<std::string> & BC_var){
    // automatic fill-in
    NonlinearSystemBase & _nl = _fe_problem.getNonlinearSystemBase();
    ...
    {
        std::vector<std::string> tmp = AssembleFlux3::split_string(*str_tmp, '-');
...
AssembleFlux3::split_string(const std::string & s, char delim)
{
