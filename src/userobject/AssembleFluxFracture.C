...
#include "AssembleFluxFracture.h"
#include <iostream>
#include <string>
#include "FEProblem.h"
...
registerMooseObject("parrotApp", AssembleFluxFracture);


template <>
InputParameters
validParams<AssembleFluxFracture>()
{
    ...
    params.addRequiredParam<Real>("matrix_1","matrix_1");
    params.addRequiredParam<Real>("matrix_2","matrix_2");
    ...
    params.addRequiredParam<std::vector<boundary_id_type>>("boundary_M_bc","boundary_M_bc");
...
AssembleFluxFracture::AssembleFluxFracture(const InputParameters & parameters) :
...
_boundary_M_ids(getParam<std::vector<boundary_id_type>>("boundary_M_bc")),
_value_N_bc(getParam<std::vector<Real>>("value_N_bc")),
_value_D_bc(getParam<std::vector<Real>>("value_D_bc")),
_matrix_value_1(getParam<Real>("matrix_1")),
_matrix_value_2(getParam<Real>("matrix_2")),
...
void
AssembleFluxFracture::initialize()
{
...
void
AssembleFluxFracture::ComputeFlux()
{
    ...
    DofMap   const & dof_map = _fe_problem.getNonlinearSystemBase().dofMap();
    DofMap   const & dof_map_2 = _fe_problem.getNonlinearSystemBase().dofMap();
...
//    PetscMatrix<Number> _boundary_matrix_2(_fe_problem.es().get_mesh().comm());
//
...
//    _boundary_matrix_2.attach_dof_map(dof_map);
//    _boundary_matrix_2.init();
    ...
    for ( ; el != end_el; ++el)
    {
        ...
        permeability_face.assign(qrule->n_points(),localPermeability);
        ...
        for (unsigned int i=0; i<phi.size(); i++)
            for (unsigned int j=0; j<phi.size(); j++)
                for (unsigned int qp=0; qp<qrule->n_points(); qp++)
                {
                    if (permeability.at(qp) == _matrix_value_1 || permeability.at(qp) == _matrix_value_2)
            ...
            if (elem->neighbor_ptr(side) == nullptr)
            {
                ...
                if(_hasMeshModifier)
                {
                    for (unsigned int qp=0; qp<qface->n_points(); qp++)
                    {
                        if ( _fractureUserObject_ptr[0].isInside(q_points[qp]) )
                        {
                            permeability_face.at(qp)=_vector_value.at(_vector_value.size()-1);
                        }
                    }
                }
                
                for(int k =0; k < _boundary_N_ids.size(); k++){
                   // std::cout<<mesh.get_boundary_info().boundary_id (elem, side)<<std::endl;
                    if (mesh.get_boundary_info().has_boundary_id (elem, side, _boundary_N_ids[k])) // Apply a traction on the right side
                    {
                        ...
                        for (unsigned int qp=0; qp<qface->n_points(); qp++)
                        {
                            ...
                            {
                                if(permeability_face.at(qp) == _matrix_value_1 || permeability_face.at(qp) == _matrix_value_2)
                                {
                                     re_1(i) += JxW_face[qp] * -1.0 * _value_N_bc.at(k) * phi_face[i][qp];
        ...
        //re_2.zero();
        dof_map.constrain_element_matrix_and_vector (ke_1, re_1, dof_indices);
        _stiffness_matrix_1.add_matrix (ke_1, dof_indices);
        _neum_flux1.add_vector(re_1, dof_indices);
    }
   
    _stiffness_matrix_1.close();
    _neum_flux1.close();
    
    MeshBase::const_element_iterator           el_2 = mesh.active_local_elements_begin();
    MeshBase::const_element_iterator const end_el_2 = mesh.active_local_elements_end();
    
    permeability.clear();
    permeability_face.clear();
    
    for ( ; el_2 != end_el_2; ++el_2)
    {
        const Elem * elem = *el_2;
                ...
                for(int k =0; k < _boundary_N_ids.size(); k++){
                    if (mesh.get_boundary_info().has_boundary_id (elem, side, _boundary_N_ids[k])) // Apply a traction on the right side
                    {
                        for (unsigned int qp=0; qp<qface->n_points(); qp++)
                        {
                            ...
                            {
                                if (permeability.at(qp) != _matrix_value_1 && permeability.at(qp) != _matrix_value_2)
                                    re_2(i) += JxW_face[qp] * -1.0 * _value_N_bc.at(k) * phi_face[i][qp];
                            }
                        }
                    }
                }
        ...
        //re_2.zero();
        dof_map.constrain_element_matrix_and_vector (ke_2,re_2, dof_indices);
        _stiffness_matrix_2.add_matrix (ke_2, dof_indices);
        _neum_flux2.add_vector(re_2, dof_indices);
    ...
    _console<<"_neum_flux2 =>"<<_neum_flux2.sum()<<std::endl;
    _console<<"_neum_flux1 =>"<<_neum_flux1.sum()<<std::endl;
    ...
    _console<<"_Boundary Mass begin" <<std::endl;
     std::cout<<"new_id"<<mesh.get_boundary_info().get_id_by_name("new_side_set")<<std::endl;
    ...
    for ( ; el_3 != end_el_3; ++el_3)
    {
        ...
        for (auto side : elem->side_index_range())
        {
            //if (elem->neighbor_ptr(side) == nullptr)
            {
                ...
                {
                    if (mesh.get_boundary_info().has_boundary_id (elem, side, _boundary_M_ids[k])) // Apply a traction on the right side
                    {
                        //std::cout<<"ciao"<<std::endl;
                        for (unsigned int qp=0; qp<qface->n_points(); qp++)
                        {
    ...
    _boundary_matrix.get_diagonal(_diag);
     ...
     _diag.print_matlab("diagonal_before.m");
    ...
    _diag.close();
    _diag.print_matlab("diagonal.m");
    

    _console<<"_Boundary Mass End" <<std::endl;
    ...
    AssembleFluxFracture::determine_dc_bnd_var_id(AssembleFluxFracture::split_string(_dc_var, ' '));
...
Real
AssembleFluxFracture::ComputeMaterialProprties(const Elem *elem)
{
...
void
AssembleFluxFracture::determine_dc_bnd_var_id(const std::vector<std::string> & BC_var){
    // automatic fill-in
    NonlinearSystemBase & _nl = _fe_problem.getNonlinearSystemBase();
    ...
    {
        std::vector<std::string> tmp = AssembleFluxFracture::split_string(*str_tmp, '-');
...
AssembleFluxFracture::split_string(const std::string & s, char delim)
{
