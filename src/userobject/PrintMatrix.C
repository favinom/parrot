...
registerMooseObject("parrotApp",PrintMatrix);

template <>
InputParameters
validParams<PrintMatrix>()
{
...
PrintMatrix::stabilize_coeffiecient()
{
    ...
    if(_fe_problem.timeStep()==1){
        ...
        PrintMatrix::determine_dc_bnd_var_id(PrintMatrix::split_string(_dc_var, ' '));
...
void
PrintMatrix::find_boundary(std::vector<int> &zero_rows, std::vector<int> &_dc_boundary_id){
...
void 
PrintMatrix::determine_dc_bnd_var_id(const std::vector<std::string> & BC_var){
    // automatic fill-in
     NonlinearSystemBase & _nl = _fe_problem.getNonlinearSystemBase();
    ...
    {
        std::vector<std::string> tmp = PrintMatrix::split_string(*str_tmp, '-');
...
PrintMatrix::split_string(const std::string & s, char delim)
{
...
void PrintMatrix::set_solution(PetscVector<Number> &correction, PetscVector<Number> &sol)
{
