...
registerMooseObject("parrotApp", SolveDiffusion2);


template <>
InputParameters
validParams<SolveDiffusion2>()
{
...
SolveDiffusion2::SolveDiffusion2(const InputParameters & parameters) :
...
void SolveDiffusion2::initialize()
{
...
/*int SolveDiffusion2::solve(EquationSystems & es)
{
...
void SolveDiffusion2::AssembleDiffusionOP(EquationSystems & _es, std::string const & system_name)
{
  ...
  for ( ; el != end_el; ++el)
  {
   ...
   Real
   SolveDiffusion2::ComputeMaterialProprties(const Elem *elem)
   {
  ...
  void SolveDiffusion2::set_solution(EquationSystems & _es)
  {
