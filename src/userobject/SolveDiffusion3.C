...
registerMooseObject("parrotApp", SolveDiffusion3);


template <>
InputParameters
validParams<SolveDiffusion3>()
{
...
SolveDiffusion3::SolveDiffusion3(const InputParameters & parameters) :
...
void SolveDiffusion3::initialize()
{
...
int SolveDiffusion3::solve(EquationSystems & es)
{
  ...
  parrotSolver->setConstantMatrix(true);
  parrotSolver->solve();
...
void SolveDiffusion3::AssembleDiffusionOP(EquationSystems & _es, std::string const & system_name)
{
  ...
  for ( ; el != end_el; ++el)
  {
   ...
   Real
   SolveDiffusion3::ComputeMaterialProprties(const Elem *elem)
   {
  ...
  void SolveDiffusion3::set_solution(EquationSystems & _es)
  {
